...
const FROM_EMAIL = process.env.FROM_EMAIL || 'noreply@mongodbtools.dev';
const FROM_NAME = process.env.FROM_NAME || 'MongoDB Tools';
...
<head>
  ...
  <title>Sign in to MongoDB Tools</title>
</head>
          ...
          <tr>
            <td style="padding: 32px 32px 24px; text-align: center; border-bottom: 1px solid rgba(0, 237, 100, 0.2);">
              <div style="display: inline-block; background: linear-gradient(135deg, #00ED64 0%, #00CC55 100%); padding: 12px 20px; border-radius: 8px; margin-bottom: 16px;">
                <span style="color: #001E2B; font-size: 20px; font-weight: 700;">MongoDB Tools</span>
              </div>
          ...
          <tr>
            <td style="padding: 32px;">
              <p style="color: rgba(255, 255, 255, 0.7); font-size: 16px; line-height: 1.6; margin: 0 0 24px;">
                Click the button below to securely sign in to MongoDB Tools. This link will expire in <strong style="color: #00ED64;">${expiresInMinutes} minutes</strong>.
              </p>
          ...
          <tr>
            <td style="padding: 24px 32px; background: rgba(0, 0, 0, 0.2); text-align: center; border-top: 1px solid rgba(255, 255, 255, 0.1);">
              <p style="color: rgba(255, 255, 255, 0.4); font-size: 12px; line-height: 1.5; margin: 0;">
                MongoDB Tools - Build aggregation pipelines, visualize schemas, and create forms<br>
...
`;
  ...
  try {
    ...
    const info = await transport.sendMail({
      ...
      to,
      subject: 'Sign in to MongoDB Tools',
      text,
      html,
    });
...
`;
  ...
  try {
    ...
    await transport.sendMail({
      from: `"${FROM_NAME}" <${FROM_EMAIL}>`,
      to,
      subject: 'New passkey added to your MongoDB Tools account',
      html,
    });
    return true;
  } catch (error) {
